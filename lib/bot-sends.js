...
async function loadAll(force) {
  ...
  try {
    ...
    // Keep any in-memory rows the blob does not yet have (race with just-recorded sends).
    for (const [k, memRows] of prev.entries()) {
      const blobRows = map.get(k) || [];
      const merged = blobRows.slice();
      for (const r of memRows || []) {
        if (!r) continue;
        const dup = merged.some((b) =>
          b && r
          && b.at === r.at
          && b.fp === r.fp
          && b.messageId === r.messageId
          && b.crmId === r.crmId
        );
...
async function recordSend(chatKey, info) {
  ...
  return withLock(async () => {
    // Merge blob + memory so a concurrent load cannot drop this row.
