...
function field(o, names) {
  for (const key of Object.keys(o)) {
    ...
    for (const n of names) if (k === n.toLowerCase()) return o[key];
  }
  return "";
}
...
// Split a multi-item cell (advantages) on newlines, ";" or "|".
...
function buildHouseText(h, routes) {
  ...
  if (rr.length) {
    ...
    for (const r of rr) {
      const mins = r.minutes ? ` — ~${String(r.minutes).replace(/\s*мин\.?$/i, "")} мин` : "";
...
async function getData() {
  ...
  try {
    const [roomsCsv, housesCsv, routesCsv, photosCsv] = await Promise.all([
    ...
    if (housesCsv !== null && routesCsv !== null) {
      ...
      // Publish a live block PER HOUSE only if THAT house has >=1 route. A house
      // whose routes are missing (accident / not yet filled) is deferred to its
      // committed dom-*.md, so we never override a house and drop its route
      // grounding. Empty Маршруты tab ⇒ routed is empty ⇒ all houses deferred.
      const skipped = houseRows.filter((h) => !routed.has(h.name)).map((h) => h.name);
      if (skipped.length) console.warn("sheets: no routes for " + skipped.join(", ") + " — deferred to committed baseline");
      houseTexts = houseRows.filter((h) => routed.has(h.name)).map((h) => buildHouseText(h, routes));
