...
function classifyMoveIn(raw) {
  ...
  // Stem forms: сентября/сентябре, октября/октябре, etc.
  if (
    /брон|учебн|семестр|сентябр|октябр|ноябр|декабр|на год|year|reserv/i.test(low)
  ) {
    return { moveIn: s, moveInKind: "booking" };
  }
  if (
    /июн|июл|август|сразу|сейчас|скор|ближай|лет|лето|move.?in|immediate|soon/i.test(low)
  ) {
...
// "1" / "дом 2" / "Дом 2" → "Дом 2" (rooms/routes/photos match by the same key).
function houseName(v) {
  const s = String(v).trim();
  if (!s) return "";
  const m = /^(?:дом\s*)?(\d+)$/i.exec(s);
  if (m) return `Дом ${m[1]}`;
  return s.replace(/^(дом)\s+(\d+)\b/i, (_, _w, n) => `Дом ${n}`);
}
...
function mapRooms(objs) {
  ...
  for (const o of objs) {
    ...
    const row = {
      ...
      free: num(field(o, ["Свободно", "Свободных", "Свободных мест", "free"])),
      price: num(field(o, ["Цена", "Цена тг", "Цена ₸", "price"])),
...
async function fetchIgnoredFromSheet() {
  ...
  // On fetch failure keep the previous list (fail-closed for residents), don't
  // cache an empty blocklist for TTL_MS.
  if (csv === null) {
    if (Array.isArray(ignoreCache.phones)) return ignoreCache.phones;
    return [];
  }
