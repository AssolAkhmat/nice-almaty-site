 ...
 *
 * IMPORTANT: some Wazzup webhook echoes of our own Admin API sends arrive with
 * isEcho=true but WITHOUT authorName/authorId and WITHOUT our crmMessageId.
 * Those false positives were muting the chat right after every bot reply.
 * Real Phone/iframe managers include authorName when isEcho=true (Wazzup docs).
 */
function isPhoneOutbound(m, isGroupFn) {
  ...
  if (m.isEcho !== true) return false;
  // Reject Admin API false echoes: isEcho but no human author.
  const author = String(m.authorName || "").trim();
  if (!author && !m.authorId) return false;
  return true;
}

/** Human manager on Phone — never Admin API, never Wazzup UI alone. */
function isHumanOutbound(m, isGroupFn) {
  return isPhoneOutbound(m, isGroupFn);
}
...
function classifyOutbound(m, isGroupFn) {
  ...
  if (m.sentFromApp === true) return "wazzup_ui"; // logged only — does NOT mute
  return "other";
}
...
function noteManagerActivity(messages, opts) {
  ...
  // Mute ONLY when Phone writes (isEcho). Wazzup UI / Admin API never mute.
  const muted = [];
  for (const m of messages || []) {
    ...
    if (!id) continue;
    const row = muteChat(id, "phone", normalizePhone);
    if (row) muted.push(Object.assign({ kind: "phone" }, row));
  }
  return muted;
}
