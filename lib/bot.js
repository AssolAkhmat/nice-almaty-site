...
const META_LEAK_RE = /языков\w*\s+модел|deepseek|gpt-?\d|база\s+данных|стек\s+технолог|внутренн\w+\s+(?:база|информац|детал|файл|правил)|промпт|нейросет/i;
...
async function ask(opts) {
  ...
  try {
    ...
    if (!text) return { reply: fallback, ok: false, reason: "empty_reply", attachments: [] };
