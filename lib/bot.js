...
function buildSystemPrompt(opts) {
  ...
  const lines = [
    ...
    "FORMAT — reply in PLAIN TEXT only (this is sent over WhatsApp). Do NOT use Markdown or HTML: no **bold**, no _italics_, no headings (#), no backticks, no [text](url) links, and no <a>/<...> tags. Write any link as a plain URL, e.g. https://wa.me/77770739990 . Short line breaks and a few soft emoji are fine (do not overuse).",
    "",
    ...
    "- Kazakh: ALWAYS open with «Сәлеметсіз бе» when a greeting is appropriate. NEVER write «Сәлем», «Салем», «Сәлеметсің бе», «Қайырлы таң» alone as a casual hello, or any informal «сен» forms. Prefer formal «Сіз» throughout.",
    "- Russian: greet with «Здравствуйте», never «Привет», «Здарова», «Хей», «Хай». Prefer «Вы».",
    "- English: «Hello» / «Good afternoon», never «Hey» / «Hiya» / slang.",
...
function enforcePoliteGreeting(s) {
  let t = String(s || "");
  // Line-start casual Kazakh «Сәлем» / Latin-ish «Salem» → formal
  t = t.replace(/(^|\n)\s*(С[әа]лем|Салем|Salem)\s*[!.]?\s*/giu, "$1Сәлеметсіз бе! ");
...
async function ask(opts) {
  ...
  const messages = [
    { role: "system", content: buildSystemPrompt(Object.assign({}, opts, { live })) },
  ...
  try {
    ...
    const trimmed = cleanReply(reply && reply.trim());
