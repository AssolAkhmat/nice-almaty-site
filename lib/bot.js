...
function buildSystemPrompt(opts) {
  ...
  const lines = [
    ...
    "- If the university name is incomplete, mistyped or ambiguous (e.g. «кому», «крм», 2–6 letters), do NOT guess (КБТУ/КРМУ/…). Ask ONE short clarification for the full name — no long house dump yet.",
    "- If the customer corrects the university in the next message, answer about the corrected university only; do not repeat the previous wrong recommendation.",
    "",
    "NO AI SLOP (mandatory):",
    "- Never end with empty filler like «если у вас есть вопросы — напишите», «дайте знать», «я с радостью помогу», «просто напишите», «обращайтесь», «буду рад помочь», «чем ещё могу помочь», «always happy to help», «feel free to ask».",
    "- Do not pad answers with gratitude loops («спасибо за вопрос», «отличный вопрос», «понимаю ваш интерес») unless the customer is upset and needs empathy.",
    "- After answering, either stop OR ask ONE concrete next question (university / gender / move-in date). Never a vague «есть вопросы?» closer.",
  ];
  ...
  lines.push(
    ...
    "",
    "AVAILABILITY:",
    "- LIVE ROOM AVAILABILITY rows have: house, gender (муж/жен), roomType, floor, totalPlaces, free (open spots), price (KZT/month), status ('свободно' = free, 'занято' = taken, 'предварительная бронь' = tentatively booked), moveIn (period text from the sheet), moveInKind ('soon' | 'booking' | 'other').",
    "- moveInKind is CRITICAL — two different offers:",
    "  • 'soon' (e.g. moveIn «На июль-август») = can move in soon / this summer. Offer these when the student wants to заехать сразу, сейчас, в июле, в августе, на лето.",
    "  • 'booking' (e.g. moveIn «Для брони на учебный год») = reservation for the academic year, NOT immediate check-in. Offer these when they ask про бронь, забронировать, на учебный год, с сентября, на семестр.",
    "- ALWAYS say the move-in period in your answer (from moveIn). Never mix: do not present a 'booking' row as «можно сразу заселиться», and do not present a 'soon' row as the only option for academic-year booking if the student clearly wants the school year.",
    "- If the student does not say when they want to move in, ASK: сразу / июль–август, or бронь на учебный год — then filter rows by moveInKind.",
    "- This list is NOT exhaustive. If a house/room isn't in it, don't say 'no places' — availability depends on the move-in date.",
    ...
    "- If a house is full for their period, proactively suggest other houses that still have free spots for the SAME moveInKind.",
  ...
  if (booking) {
    lines.push(
      ...
      "- First clarify timing: immediate/summer (moveInKind 'soon') vs academic-year reservation (moveInKind 'booking'), then only offer matching free rows.",
      "- Help the student pick a house that has free places for their budget/university AND the correct move-in period.",
      "- To reserve, explain: they come to view the house, then hold a spot with a prepayment. Collect their preferred house, move-in period (soon vs academic year), and university.",
      "- You CANNOT finalize a reservation in the system yourself — after collecting the details, tell them a Nice Almaty manager will confirm and finalize shortly" +
        (onWhatsApp
          ? " in this same WhatsApp chat, and ONLY THEN append [МЕНЕДЖЕР] so managers see the chat."
          : ", and share WhatsApp https://wa.me/77770739990 for anything urgent."),
  ...
  if (onWhatsApp) {
    lines.push(
      "",
      "HUMAN HANDOFF (WhatsApp / Wazzup) — controls the unread badge:",
      "- Append on its OWN last line exactly [МЕНЕДЖЕР] ONLY when a human must take over now: deposit/discount exceptions, exact address/viewing arrange, finalize payment, complaints, or you already collected booking details and are transferring the chat to a manager.",
      "- When you write [МЕНЕДЖЕР], also tell the customer a manager will continue in this same chat.",
      "- Do NOT write [МЕНЕДЖЕР] for ordinary answered questions (prices, free places, move-in period, routes, FAQ, clarifying questions). Those chats must stay answered/read for managers."
    );
  }
...
// Cut generic AI closer fluff that models love to append.
function stripAiSlop(s) {
  let t = String(s || "").trim();
  if (!t) return t;
  const paragraphs = t.split(/\n{2,}/);
  const slopPara = /^(если\s+у\s+вас\s+есть\s+вопрос|если\s+есть\s+вопрос|если\s+появятся\s+вопрос|есть\s+ещё\s+вопрос|чем\s+ещё\s+могу\s+помочь|буду\s+рад[а]?\s+помочь|с\s+радостью\s+помогу|просто\s+напишите|дайте\s+знать|обращайтесь|always\s+happy\s+to\s+help|feel\s+free\s+to\s+ask|let\s+me\s+know\s+if|егер\s+сұрақ|қосымша\s+сұрақ)/iu;
  const softSlopLine = /(?:если\s+у\s+вас\s+есть\s+вопрос[^.!?\n]{0,80}|я\s+с\s+радостью\s+помогу[^.!?\n]{0,60}|просто\s+напишите[^.!?\n]{0,80}|дайте\s+знать[^.!?\n]{0,60}|чем\s+ещё\s+могу\s+помочь[^.!?\n]{0,40}|feel\s+free\s+to\s+ask[^.!?\n]{0,40}|let\s+me\s+know\s+if\s+you\s+have[^.!?\n]{0,60})/giu;

  // Drop trailing paragraphs that are pure filler.
  while (paragraphs.length > 1 && slopPara.test(paragraphs[paragraphs.length - 1].trim())) {
    paragraphs.pop();
  }
  t = paragraphs.join("\n\n");

  // Also scrub filler sentences that sit at the end of the last paragraph.
  t = t.replace(softSlopLine, "");
  t = t.replace(/[ \t]+\n/g, "\n").replace(/\n{3,}/g, "\n\n");
  t = t.replace(/\s+([,.!?;:])/g, "$1").replace(/[ \t]{2,}/g, " ").trim();
  // Clean orphaned junctions after sentence removal.
...
// Questions about AI / stack / "cheap model" / internal systems — never answer.
// Use [а-яё…] instead of \\w — JS \\w is ASCII-only without unicode property escapes.
const META_QUESTION_RE = new RegExp(
  [
    ...
    "промпт",
    "api\\s*ключ",
    "деш[её]в[а-яё]*\\s+модел",
    "модел[а-яё]*.{0,40}обработ",
    "долг[а-яё]*\\s+обрабат",
    "what\\s+model",
    "tech\\s*stack",
    "are\\s+you\\s+an?\\s+ai",
    ...
    "вы\\s+бот",
    "ты\\s+ии",
    "искусственн[а-яё]+\\s+интеллект",
  ].join("|"),
  "iu"
);

const META_LEAK_RE = /языков[а-яёa-z]*\s+модел|deepseek|gpt-?\d|база\s+данных|стек\s+технолог|внутренн[а-яёa-z]+\s+(?:база|информац|детал|файл|правил)|промпт|нейросет/iu;

const META_REDIRECT = {
  ru: "Я помогаю с заселением в студенческие дома Nice Almaty — свободные места, цены и дома рядом с вузом. Подскажите, пожалуйста, в каком университете учитесь и когда планируете заехать?",
  kz: "Мен Nice Almaty студент үйлеріне орналасуға көмектесемін — бос орындар, бағалар және вузға жақын үйлер. Қай университетте оқисыз және қашан кіргіңіз келеді?",
  en: "I help with Nice Almaty student housing — free places, prices, and houses near universities. Which university do you study at, and when do you plan to move in?",
};

function isMetaQuestion(text) {
  return META_QUESTION_RE.test(String(text || ""));
}

function replyLeaksMeta(text) {
  return META_LEAK_RE.test(String(text || ""));
}

// [МЕНЕДЖЕР] — private signal for Wazzup to keep the "unanswered" badge so a
// human sees the chat. Stripped before the customer receives the text.
// ONLY the explicit marker counts (phrase-matching caused false unread badges).
const MANAGER_MARKER_RE = /\[\s*МЕНЕДЖЕР\s*\]/gi;

function stripManagerMarker(text) {
  const raw = String(text || "");
  // Do not reuse a /g RegExp with .test() — lastIndex would break .replace().
  const flagged = /\[\s*МЕНЕДЖЕР\s*\]/i.test(raw);
  const cleaned = raw.replace(MANAGER_MARKER_RE, "").replace(/\n{3,}/g, "\n\n").trim();
  return { text: cleaned, needsManager: !!flagged };
}
...
async function ask(opts) {
  ...
  // Hard block: never let the model discuss AI/stack/internals.
  if (isMetaQuestion(message)) {
    const redirect = META_REDIRECT[lang] || META_REDIRECT.ru;
    return { reply: redirect, ok: true, attachments: [], needsManager: false, reason: "meta_blocked" };
  }
  ...
  try {
    ...
    trimmed = stripAiSlop(trimmed);
    if (!trimmed) return { reply: fallback, ok: false, reason: "empty_reply", attachments: [] };
    // If the model still bleeds meta jargon, replace with the housing redirect.
    if (replyLeaksMeta(trimmed)) {
      return {
        reply: META_REDIRECT[lang] || META_REDIRECT.ru,
        ok: true,
        attachments: [],
        needsManager: false,
        reason: "meta_scrubbed",
      };
    }
    ...
    const { text: afterPhotos, houses } = stripPhotoMarkers(trimmed);
    const { text, needsManager } = stripManagerMarker(afterPhotos);
    ...
    // Marker-only replies (photos and/or handoff) must not fall through to the
    // "assistant unavailable" fallback — that drops attachments and the badge.
    if (!text) {
      if (!houses.length && !needsManager) {
        return { reply: fallback, ok: false, reason: "empty_reply", attachments: [] };
      }
      const filler = needsManager
        ? (channel === "whatsapp"
          ? "Передал ваш вопрос менеджеру — он продолжит в этом чате."
          : "Передал ваш вопрос менеджеру — он свяжется с вами.")
        : (channel === "whatsapp" ? "Отправляю фото." : "Отправляю фото дома.");
      return { reply: filler, ok: true, attachments, needsManager };
    }
    return { reply: text, ok: true, attachments, needsManager };
  } catch (e) {
