...
const HANDOFF_PHRASE_RE = /передам[^\n.]{0,80}менеджер|менеджер[^\n.]{0,80}(?:свяжется|ответит|продолж)|ожидайте[^\n.]{0,40}ответ|manager will (?:reply|continue|contact)/i;

function stripManagerMarker(text) {
  const raw = String(text || "");
  const flagged = MANAGER_MARKER_RE.test(raw) || HANDOFF_PHRASE_RE.test(raw);
